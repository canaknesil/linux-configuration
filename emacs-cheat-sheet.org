...
Double spaces ends an Emacs sentence, which I don't use. I use a dot
followed by a single space. Make sentence commands recognize my
sentences. But this would have a disadvantage because there would be
no way to distinguish sentece endings and abreviations, etc.
...
Verilog, enables this by default, which is very anoying.
