...
Switch to occur buffer after creation. Splitting the window is
generally followed by another command such as find-file or
switch-to-buffer. There are commands that can achieve both steps at
once such as find-file-other-window or switch-to-buffer-other-window,
which also select the other windows automatically. Rather than
changing the way window selection functions, I suggest learning the
available commands, and Emacs' logic behind window selection.
...
Programming language specific behavior after hitting return after an
opening parenthesis, e.g. in C, put semicolon after the closing
parenthesis in function definitions. Maybe with Yasnippet?

Julia support for hideshow.
...
Bad face when hovering on highlighted text. hl-line overwrites the
highlight color.
...
Todo in emacs-base.org and emacs.org.
