...
current buffer. Have another command that keeps the old buffer. For
now use file-file followed by insert-file.

Change tiling of windows from vertical to horizontal and vice
versa. Try: transpose-frame package
...
Re-define custom key bindings in form C-c <char>. This is reserved for
users and Emacs won't overwrite it in the future releases.
