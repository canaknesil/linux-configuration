...
#+BEGIN_SRC emacs-lisp
  ...
  (defun configure-font (font)
    ...
    ;; Better font for key bindings, y-or-n, markdown inline code,
    ;; etc. These faces inherit from fixed-pitch and fixed-pitch-serif,
    ;; which default to Monospace on Windows. Setting font as below
    ;; doesn't take effect with Emacs server.
    (when (eq system-type 'windows-nt)
